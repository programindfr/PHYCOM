...
void affichage_luminosite(float voltage)
{
  ...
  {
    LcdString("Lumineux");        // 4-2
  }
  else if (voltage > 0.1)
  {
    LcdString("Intermediaire");   // 2-0.1
  }
  else if (voltage > 0.03)
  {
    LcdString("Sombre");          // 0.1-0.03
  }
  else
  {
    LcdString("Tres sombre");     // 0.03-0
  }
}
...
{
  // Mesures
  // la diode pour la lumière
  int sensorLuminValue = analogRead(A0);
